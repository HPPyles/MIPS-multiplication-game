...
winCheckd1:	
	...
	li $t0, 0 # ***t0 marks beginning of each diagOne***
...
# Check beginning of diagonal, then next 3 values. Do the same for one or both of the next 2 values. Jump to beginning of next diagOne.
...
diagOneLoop: 
	beq $a1, 1, diagOneMorePCheck
	beq $a1, 2, diagOneMoreECheck
	...
	beq $a1, 1, diagOneMorePCheck # If player marker found in diagonal,
	beq $a1, 2, diagOneMoreECheck # If enemy marker found in diagonal, 
	
	addi $s2, $s2, 28 # Next in diagonal
	lw $t1, 0($s2)
	bge $t1, 35, returnMain
	addi $t4, $t4, 1 # moved down 1  
	bge $t4, 7, nextDiagOne # If we've already checked 6 values, move to next 
	
	bge $t8, 3, nextDiagOne
	j diagOneLoop
...
nextDiagOne:
	...
	addi $t0, $t0, 4 # Start of diagOne marker moved to next diagOne index
	la $s2, numbers # Reset $s2 to beginning of array
	add $s2, $s2, $t0 # Move $s2 to current start of diagOne pointer
	lw $t1,0($s2) # t1 is value at beginning of the diagOne
	li $t4, 0
...
diagOneMorePCheck:
	...
	addi $s2, $s2, 28 # Next value in diagOne
	...
	beq $a1, 1, diagOneMorePCheck # If the next value has player marker, check value after that for continuation
	#else
	addi $s2, $s2, 28 # Change to next value in diagOne
	lw $t1, 0($s2)
	addi $t4, $t4, 1 # moved down 1  
	bge $t4, 7, nextDiagOne # If we've already checked 6 values, move to next 
	
	bge $t8, 3, nextDiagOne # Go to next diagOne if finishing check from 3rd starting point in diagOne
	j diagOneLoop
...
diagOneMoreECheck: 
	...
	addi $s2, $s2, 28 # Next value in row
	lw $t1, 0($s2) # put current array value in $t1
	addi $t4, $t4, 1 # moved down 1  
	bge $t4, 7, nextDiagOne # If we've already checked 6 values, move to next 
	...
	beq $a1, 2, diagOneMoreECheck # If the next value has enemy marker, check value after that for continuation
