...
winCheckVertical:	
	...
	li $t0, 0 # ***t0 marks beginning of each column***
...
# +++ CHECKING ALL POSSIBLE COLUMN WINS+++
# Check beginning of column, then next 3 values. Do the same for one or both of the next 2 values. Jump to beginning of next column.
# Repeat until out of columns
columnCheck: 
	la $s2, numbers # Start pointer at beginning of array
	lw $t1, 0($s2) # Start at 1st array value
	
columnLoop: 
	beq $a1, 1, columnMorePCheck
	beq $a1, 2, columnMoreECheck
	...
	beq $a1, 1, columnMorePCheck # If player marker found in row,
	beq $a1, 2, columnMoreECheck # If enemy marker found in row, 
	
	addi $s2, $s2, 24 # Next in column
	lw $t1, 0($s2)
	addi $t4, $t4, 1 # moved down 1  
	bge $t4, 7, nextColumn # If we've already checked 6 values, move to next 
...
nextColumn:
	...
	addi $t0, $t0, 4 # Start of column marker moved to next column index
	la $s2, numbers # Reset $s2 to beginning of array
	add $s2, $s2, $t0 # Move $s2 to current start of column pointer
	lw $t1,0($s2) # t1 is value at beginning of the column
	li $t4, 0
...
columnMorePCheck:
	...
	addi $s2, $s2, 24 # Next value in column
	...
	beq $a1, 1, columnMorePCheck # If the next value has player marker, check value after that for continuation
	#else
	addi $s2, $s2, 24 # Change to next value in column
	lw $t1, 0($s2)
	addi $t4, $t4, 1 # moved down 1  
	bge $t4, 7, nextColumn # If we've already checked 6 values, move to next 
	
	bge $t8, 3, nextColumn # Go to next column if finishing check from 3rd starting point in column
	j columnLoop
...
columnMoreECheck: 
	...
	addi $s2, $s2, 24 # Next value in row
	lw $t1, 0($s2) # put current array value in $t1
	addi $t4, $t4, 1 # moved down 1  
	bge $t4, 7, nextColumn # If we've already checked 6 values, move to next 
	...
	beq $a1, 2, columnMoreECheck # If the next value has enemy marker, check value after that for continuation
